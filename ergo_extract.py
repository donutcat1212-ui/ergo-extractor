...
Версия 4:
- отдельно считает первичные и выписные;
- игнорирует файлы с "отчет" в названии;
- при наличии выписки игнорирует первичку того же случая;
- не пишет предупреждения в основную таблицу;
- учитывает отметки "оценить/выявить затруднительно";
- исправляет кейсы, где код вроде d5400 может быть разорван в таблице.
"""
...
VERSION = "ERGO EXTRACT V4.0"
DEFAULT_ROOT_DIR = r"\\fccps.local\dfs\ОМР ЦНС1\Эрготерапия\Эрготерапия 23-25г"
DEFAULT_OUTPUT_XLSX = r"ergotherapy_extract_result_v4.xlsx"
DIFFICULT_VALUE = "затруднительно"
...
FIELD_DEFINITIONS = [
    ...
    ("d5100", "Мытье частей тела", "paired"),
...
DIFFICULT_RE = re.compile(r'(оценить|выявить).*?затруднительно|затруднительно', re.I)
...
def normalize_patient_name(patient: str) -> str:
    ...
    return re.sub(r'\s+', ' ', patient).strip()
...
def should_skip_file(filename: str) -> bool:
    low = normalize_match(filename)
    return low.startswith("~$") or "отчет" in low or not low.endswith(".docx")


def iter_docx_files(root_dir: str) -> Tuple[List[str], List[str]]:
    primaries, discharges = [], []
    for cur_root, _, files in os.walk(root_dir):
        low_root = normalize_match(cur_root)
        for fn in files:
            if should_skip_file(fn):
                continue
            full = os.path.join(cur_root, fn)
            if "1.первич" in low_root or "первич" in low_root:
                primaries.append(full)
            elif "2.выпис" in low_root or "выпис" in low_root or "повторн" in low_root:
...
def extract_code_from_row(row: List[str]) -> Tuple[str, int]:
    if not row:
        return "", 0
    first = canonical_code(row[0])
    if CODE_RE.match(first.replace("е", "e")):
        return first, 0
    if len(row) > 1:
        maybe = canonical_code((row[0] or "") + (row[1] or ""))
        if CODE_RE.match(maybe.replace("е", "e")):
            return maybe, 1
        first_norm = canonical_code(row[0])
        second_norm = normalize_match(row[1])
        if re.match(r'^[deе]\d{3}$', first_norm.replace("е", "e")) and re.fullmatch(r'\d', second_norm):
            glued = canonical_code(first_norm + second_norm)
            if CODE_RE.match(glued.replace("е", "e")):
                return glued, 1
    return "", 0
...
def parse_icf_profile_table(table_rows: List[List[str]]) -> Dict[str, Tuple[str, str]]:
    ...
    date_cols = []
    for idx, cell in enumerate(table_rows[2]):
        if DATE_RE.search(cell or ""):
            date_cols.append(idx)
    if not date_cols:
        return out

    for row in table_rows[4:]:
        code, shift = extract_code_from_row(row)
        if not code:
            continue
        field = FIELD_BY_CODE.get(code)
        if not field:
            continue
        cols = [c + shift for c in date_cols]
        vin = parse_value_text(row[cols[0]]) if cols[0] < len(row) else None
        vout = parse_value_text(row[cols[1]]) if len(cols) > 1 and cols[1] < len(row) else None
        out[field.field_id] = (vin or "", vout or "")
    return out


def metric_missing(metric: Tuple[str, str]) -> bool:
    return not metric[0] and not metric[1]
...
def parse_narrative_scale_values(full_text: str, doc_kind: str) -> Dict[str, Tuple[str, str]]:
    ...
    patterns = {
        "Шкала SULCS": [
            r'емкостн\w* тест верхн\w* конечн\w*\s*sulcs\s*[:\-]?\s*([^\n]+)',
            r'шкала\s*sulcs\s*[:\-]?\s*([^\n]+)',
        ],
        "Шкала FIM": [
            r'шкала функциональн\w* независим\w*.*?fim.*?[:\-]?\s*([^\n]+)',
            r'шкала\s*fim\s*[:\-]?\s*([^\n]+)',
        ],
        "Шкала EQ-5D": [
            r'опросник\s*eq-?5d.*?[:\-]?\s*([^\n]+)',
            r'шкала\s*eq-?5d\s*[:\-]?\s*([^\n]+)',
        ],
    }
    ...
    for disp, plist in patterns.items():
        ...
        for p in plist:
            ...
            if not m:
                continue
            candidate = parse_value_text(m.group(1))
            if candidate:
                found = candidate
                break
        if found:
...
def parse_single_doc(path: str) -> ParsedDoc:
    ...
    if not patient:
        ...
        warnings.append("Ф.И.О. не найдено в тексте; использовано имя файла")
    ...
    # Separate structural warnings from stats; keep them only on warnings sheet.
    if not activity:
        pass
    if not any(a or b for a, b in metrics.values()):
        warnings.append("Не извлечены целевые показатели")
...
def export_xlsx(docs: List[ParsedDoc], out_path: str):
    ...
    headers = [
        ...
        "Характер занятий", "Источник файла", "Число извлеченных показателей"
    ]
    for f in FIELDS:
        if f.kind == "paired":
...
def main():
    ...
    print("=" * 72)
    print(f"{VERSION} | режим: первичные + выписные | приоритет: выписка | skip: отчет")
    print("=" * 72)
